...
async function sendResendEmail(input: {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { sent: false, error: detail || `Email provider returned ${response.status}.` };
    }
...
export async function sendEnquiryEmails(payload: EnquiryEmailPayload): Promise<EmailResult> {
  ...
  const clientHtml = `
    <div style="font-family:Arial,sans-serif;background:#f7f3ec;padding:32px;color:#1b0b20">
      <div style="max-width:640px;margin:0 auto;background:#fff;padding:36px;border:1px solid #e5dccd">
        ...
        <p style="line-height:1.7;color:#5c5360">If your request is urgent, you can also contact LIDYA directly by telephone or WhatsApp through the website.</p>
  ...
  if (!clientResult.sent) return clientResult;
  if (!adminResult.sent) return { sent: false, error: `Client confirmation sent, admin notification failed: ${adminResult.error || "unknown error"}` };
  return { sent: true };
}
