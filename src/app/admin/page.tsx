...
export default function AdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/auth/session", { cache: "no-store" })
      ...
      .then((payload) => {
        if (payload?.authenticated) setUser(payload.user);
      })
  ...
  if (loading || !user) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#0d0710] text-[#c8a96a]">
        <div className="text-center">
          <Image src="/images/lidya-logo.png" alt="LIDYA" width={260} height={90} className="mx-auto h-12 w-auto object-contain" />
          <p className="mt-5 text-xs uppercase tracking-[0.3em] text-white/30">Secure session check</p>
        </div>
      </main>
    );
  }
      ...
      <div className="flex min-h-screen">
        <aside className={`${mobileNav ? "fixed inset-y-0 left-0 z-50 flex" : "hidden"} w-[275px] shrink-0 flex-col border-r border-[#c8a96a]/12 bg-[#120817] lg:sticky lg:top-0 lg:flex lg:h-screen`}>
          <div className="border-b border-[#c8a96a]/12 px-6 py-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <Image src="/images/lidya-logo.png" alt="LIDYA" width={220} height={70} className="h-9 w-auto object-contain" />
                <p className="mt-3 text-[10px] uppercase tracking-[0.22em] text-white/30">Alba Resort · Administration</p>
              </div>
              <button onClick={() => setMobileNav(false)} className="text-white/40 lg:hidden">×</button>
            </div>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto px-3 py-4">
            {navItems.map((item) =>
              item.href ? (
                <Link key={item.id} href={item.href} className="mb-1 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-white/45 transition hover:bg-white/[0.035] hover:text-white/75">
                  <span className="w-5 text-center text-[#c8a96a]/75">{item.icon}</span><span>{item.label}</span>
                </Link>
              ) : (
          ...
          <div className="border-t border-[#c8a96a]/12 p-4">
            <div className="rounded-xl border border-white/7 bg-white/[0.025] p-3">
              ...
              <p className="mt-1 text-[10px] uppercase tracking-[0.16em] text-[#c8a96a]/70">{user.role === "admin" ? "Administrator" : "Staff"}</p>
              <p className="mt-1 truncate text-[10px] text-white/25">{user.email}</p>
            </div>
        ...
        <section className="min-w-0 flex-1">
          <header className="sticky top-0 z-30 flex h-[72px] items-center gap-3 border-b border-[#c8a96a]/10 bg-[#0d0710]/90 px-4 backdrop-blur-xl md:px-6 xl:px-8">
            ...
            <Link href="/" target="_blank" className="rounded-xl border border-[#c8a96a]/20 px-3 py-2.5 text-xs text-[#e8d8b5]/70">Web ↗</Link>
          </header>
          ...
          <div className="mx-auto max-w-[1500px] px-4 py-7 md:px-6 md:py-9 xl:px-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <StatCard label="Security" value="Protected" note="Supabase Auth + roles" />
              <StatCard label="Jazyky" value="15" note="Multilingual website" />
              <StatCard label="Session" value="Active" note="HTTP-only secure cookies" />
            </div>

            <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_1fr]">
              <section className="rounded-2xl border border-[#c8a96a]/15 bg-white/[0.035] p-5 md:p-6">
                <div className="flex items-center justify-between gap-3">
                  <div><p className="text-[10px] uppercase tracking-[0.24em] text-[#c8a96a]">Quick management</p><h2 className="mt-2 text-2xl">Hlavné oblasti webu</h2></div>
              ...
              <section className="rounded-2xl border border-[#c8a96a]/15 bg-white/[0.035] p-5 md:p-6">
                ...
                <h2 className="mt-2 text-2xl">Production Security</h2>
                <div className="mt-5 space-y-3">
                  {["Supabase Auth", "Admin role verification", "HTTP-only cookies", "Protected admin API", "Service role server-only"].map((item) => (
                    <div key={item} className="flex items-center justify-between rounded-xl border border-white/7 bg-black/15 px-4 py-3"><span className="text-sm text-white/70">{item}</span><span className="text-emerald-300">●</span></div>
                  ))}
                </div>
              </section>
            </div>
