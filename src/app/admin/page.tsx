...
function StatCard({ label, value, note }: { label: string; value: string; note: string }) {
  return <div className="rounded-2xl border border-[#c8a96a]/15 bg-white/[0.035] p-5"><p className="text-[10px] uppercase tracking-[0.22em] text-[#c8a96a]/70">{label}</p><p className="mt-3 text-3xl font-medium text-[#fffdf9]">{value}</p><p className="mt-2 text-xs text-white/35">{note}</p></div>;
}
...
export default function AdminPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/auth/session", { cache: "no-store" }).then(async (response) => {
      if (!response.ok) { window.location.href = "/admin/login"; return null; }
      return response.json();
    }).then((payload) => { if (payload?.authenticated) setUser(payload.user); }).finally(() => setLoading(false));
  }, []);

  const activeLabel = useMemo(() => navItems.find((item) => item.id === active)?.label ?? "Prehľad", [active]);
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1500px] px-4 py-7 md:px-6 md:py-9 xl:px-8">
            <div className="mb-7"><p className="text-[10px] uppercase tracking-[0.28em] text-[#c8a96a]">Secure control centre</p><h1 className="mt-2 text-3xl md:text-4xl">{active === "overview" ? "LIDYA Admin Dashboard" : activeLabel}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-white/45">Administrácia je chránená serverovou Supabase autentifikáciou a rolami admin/staff. Všetky položky menu sú napojené na funkčné administračné moduly.</p></div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Web" value="Online" note="Production · Vercel" /><StatCard label="Security" value="Protected" note="Supabase Auth + roles" /><StatCard label="Jazyky" value="15" note="Multilingual website" /><StatCard label="Session" value="Active" note="HTTP-only secure cookies" /></div>
            <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_1fr]">
              <section className="rounded-2xl border border-[#c8a96a]/15 bg-white/[0.035] p-5 md:p-6"><div className="flex items-center justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.24em] text-[#c8a96a]">Quick management</p><h2 className="mt-2 text-2xl">Hlavné oblasti webu</h2></div><Link href="/admin/products" className="rounded-full border border-[#c8a96a]/30 px-4 py-2 text-xs text-[#e8d8b5]">Správa produktov →</Link></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{categoryCards.map(([title, text, meta, href]) => <Link key={title} href={href} className="rounded-xl border border-white/8 bg-black/15 p-4 transition hover:border-[#c8a96a]/35 hover:bg-[#c8a96a]/5"><div className="flex items-center justify-between"><span className="text-base text-white/90">{title}</span><span className="text-xs text-[#c8a96a]">↗</span></div><p className="mt-2 text-xs leading-5 text-white/40">{text}</p><p className="mt-4 text-[10px] uppercase tracking-[0.18em] text-white/25">{meta}</p></Link>)}</div></section>
              <section className="rounded-2xl border border-[#c8a96a]/15 bg-white/[0.035] p-5 md:p-6"><p className="text-[10px] uppercase tracking-[0.24em] text-[#c8a96a]">System status</p><h2 className="mt-2 text-2xl">Production Security</h2><div className="mt-5 space-y-3">{["Supabase Auth", "Admin role verification", "HTTP-only cookies", "Protected admin API", "Service role server-only"].map((item) => <div key={item} className="flex items-center justify-between rounded-xl border border-white/7 bg-black/15 px-4 py-3"><span className="text-sm text-white/70">{item}</span><span className="text-emerald-300">●</span></div>)}</div></section>
            </div>
          </div>
        </section>
      </div>
